  ...
  render() {
    ...
    const getContent = (path) => {
      if (path === '/' || path === '/blog/beta/') {
        return (
          <span>{surveyLink}, or <a href="/blog">return to current version</a>.</span>
        );
      } else if (path === '/blog/beta/authors' || path === '/authors') {
        return (
          <span>{surveyLink}, or&nbsp;
            <a href="/voices/blogs/authors">return to current blogger profiles</a>.
          </span>
        );
      } else if (path.indexOf('subjects') !== -1) {
        return (
          <span>{surveyLink}, or&nbsp;
            <a href="/blog/subject">return to "Subjects" in the current version</a>.
          </span>
        );
        ...
        return (
          <span>{surveyLink}, or&nbsp;
            <a href={`/voices/blogs/blog-channels/${path.substring(seriesIndex)}`}>
              return to the current version of this series</a>.
          </span>
        );
        ...
        return (
          <span>{surveyLink}, or&nbsp;
            ...
            return to the current author page</a>.
          </span>
        );
      } else {
        let index = 1;
        ...
        return (
          <span>{surveyLink}, or &nbsp;
            <a href={`/blog/${path.substring(index)}`}>return to the current version
            of this blog post</a>.
            </span>
          );
      }
